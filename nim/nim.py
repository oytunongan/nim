...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        available_actions = Nim.available_actions(state)
        best_action = list(self.q.keys())[list(
            self.q.values()).index(self.best_future_reward(state))]
        greedy_best = 100 * (1 - self.epsilon)
        greedy_random = 100 * self.epsilon
        if epsilon:
            choice = random.choices(["best", "random"], weights=[greedy_best, greedy_random])[0]
            if choice == "best":
